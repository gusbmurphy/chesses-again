...
public class PawnMovementRule extends SinglePieceMovementRule {
    ...
    public static boolean verticalMovementDirectionIsOkayForColor(
            int rankDifference, PlayerColor color) {
    ...
    private static boolean moveCoordinatesAreOccupied(BoardState boardState, Move move) {
        BoardCoordinateState moveCoordinatesState =
                boardState.coordinateStates().forCoordinates(move.coordinates()).get();
        return moveCoordinatesState.isOccupied();
    }

    private static boolean pieceHasMovedAndMoveIsBeyondOneSpot(
            Move move, PieceOnBoard pieceOnBoard, Coordinates currentCoordinates) {
        return pieceOnBoard.hasMoved()
                && moveIsGreaterThanOneSpotVertically(move, currentCoordinates);
    }

    private static boolean moveIsGreaterThanTwoSpotsVertically(
            Move move, Coordinates currentCoordinates) {
        return Math.abs(currentCoordinates.rankDifferenceTo(move.coordinates())) > 2;
    }

    private static boolean moveIsGreaterThanOneSpotVertically(
            Move move, Coordinates currentCoordinates) {
    ...
    private static boolean moveIsBackwards(
            Move move, Coordinates currentCoordinates, PlayerColor color) {
        return verticalMovementDirectionIsOkayForColor(
                currentCoordinates.rankDifferenceTo(move.coordinates()), color);
    }
}
